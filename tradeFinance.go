...
const accessControlFlag bool = true
...
func (t *TF) listContracts(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	for row := range rows {
		...
		b, err := t.lc.GetStatus(stub,[]string{nextContract.ContractID})
		if err != nil {
			return nil, err
		}
...
func (t *TF) listLCsByStatus(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	for row := range rows {
		...
		b, err := t.lc.GetStatus(stub, []string{row.Columns[1].GetString_()})
		if err != nil {
			return nil, err
		}
...
//func (t *TF) Invoke(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	...
	if function == "submitLC" {
		...
		lcStatus, err := t.lc.GetStatus(stub, []string{args[0]})
		if err != nil {
			return nil, err
		}
		...
		if lc.Tag42C == "Sight" {
			t.lc.UpdateStatus(stub, []string{args[0], "PAYMENT_DUE_FROM_IB_TO_EB"})
		}
...
func (t *TF) Query(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	*/
	...
	if function == "getLC" {
		...
		b, err := t.lc.GetStatus(stub, args)
		if err != nil {
			return nil, err
		}
